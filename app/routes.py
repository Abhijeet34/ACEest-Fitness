...
def add_client():
    ...
    try:
        db.execute(
            'INSERT INTO clients (name, age, height, weight, program, membership_status) VALUES (?, ?, ?, ?, ?, ?)',
            (data['name'], data.get('age'), data.get('height'), data.get('weight'), data.get('program'), 'Active')
        )
        db.commit()
...
def add_workout():
    ...
    db.execute(
        ...
        (data['client_name'], data['date'], data['workout_type'], data.get('duration_min', 60), data.get('notes', ''))
    )
    db.commit()
